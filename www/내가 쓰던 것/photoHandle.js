// 카메라 장치를 이용하여 사진 촬영 후 base64-encoded 이미지 문자열 임시 파일 경로를 변환
...
// resolveLocalFileSystemURL() 성공 콜백 함수
function resolveOnSuccess(fileentry) {
	...
	window.requestFileSystem(LocalFileSystem.PRESISTENT, 0, function(filesysobj) {
		// 폴더가 존재하지 않으면 생성됨
		filesysobj.root.getDirectory( newfoldername, { create:true, exclusive: false }, function(directoryentry) {
